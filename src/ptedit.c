...
// ---------------------------------------------------------------------------
static ptedit_entry_t ptedit_resolve_user_ext(void *address, pid_t pid,
  ...
  //     pgdi, ptedit_vmem + root + pgdi * ptedit_entry_size);
  ...
  if (!ptedit_cast(pmd_entry, ptedit_pmd_t).size) {
    ...
    pt_entry =
        deref(pfn * ptedit_pfn_multiply + pti * ptedit_entry_size); // pt[pti];
...
// ---------------------------------------------------------------------------
ptedit_fnc void ptedit_update_kernel(void *address, pid_t pid,
                                     ptedit_entry_t *vm) {
  vm->vaddr = (size_t)address;
  vm->pid = (size_t)pid;
#if defined(LINUX)
...
// ---------------------------------------------------------------------------
ptedit_fnc void ptedit_update_user_ext(void *address, pid_t pid,
                                       ptedit_entry_t *vm,
...
#if defined(__i386__) || defined(__x86_64__) || defined(_WIN64)
  entry &= ~((1ull << PTEDIT_PAGE_BIT_PWT) | (1ull << PTEDIT_PAGE_BIT_PCD) |
             (1ull << PTEDIT_PAGE_BIT_PAT_LARGE));
  if (mt & 1)
    entry |= (1ull << PTEDIT_PAGE_BIT_PWT);
  if (mt & 2)
    entry |= (1ull << PTEDIT_PAGE_BIT_PCD);
  if (mt & 4)
    entry |= (1ull << PTEDIT_PAGE_BIT_PAT_LARGE);
#elif defined(__aarch64__)
  entry &= ~0x1c;
  entry |= (mt & 7) << 2;
#endif
...
unsigned char ptedit_extract_mt_huge(size_t entry) {
  ...
  return (!!(entry & (1ull << PTEDIT_PAGE_BIT_PWT))) |
         ((!!(entry & (1ull << PTEDIT_PAGE_BIT_PCD))) << 1) |
         ((!!(entry & (1ull << PTEDIT_PAGE_BIT_PAT_LARGE))) << 2);
#elif defined(__aarch64__)
  return (entry >> 2) & 7;
#endif
}
